    ...
    return (
      <Link href={`/article/${slug}`} className="group flex items-start gap-3 py-3 border-b border-gray-100 last:border-0">
        <div className="relative w-16 h-16 shrink-0 overflow-hidden rounded bg-gray-100">
          {article.heroMedia && (
            <Image src={imgUrl} alt={title} fill className="object-cover group-hover:scale-105 transition-transform duration-300" />
          )}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold font-hindi leading-snug text-gray-800 group-hover:text-primary line-clamp-2 transition-colors">
            {title}
          </p>
    ...
    return (
      <Link href={`/article/${slug}`} className="group flex gap-4 news-card">
        ...
        <div className="flex-1 min-w-0 py-1">
          {catName && (
            <Link href={`/${catSlug}`} className="cat-badge mb-2 inline-block" style={{ fontSize: "0.65rem" }}>
              {catName}
            </Link>
          )}
          <h3 className="font-hindi text-base font-semibold leading-snug text-gray-800 group-hover:text-primary line-clamp-2 transition-colors">
            {title}
          </h3>
          <span className="text-xs text-gray-400 mt-1 block">{time}</span>
        </div>
      ...
      <Link href={`/article/${slug}`} className="group block news-card">
        <div className="relative aspect-video overflow-hidden rounded bg-gray-100 mb-2">
          ...
          {article.breakingNews && (
            <span className="absolute top-2 left-2 live-badge text-[10px]">
              <span className="breaking-dot"></span> Breaking
            </span>
          )}
        </div>
        {catName && (
          <Link href={`/${catSlug}`} className="cat-badge mb-1 inline-block">
            {catName}
          </Link>
        )}
        <h3 className="font-hindi text-sm font-semibold leading-snug text-gray-800 group-hover:text-primary line-clamp-2 transition-colors">
          {title}
        </h3>
        <span className="text-xs text-gray-400 mt-1 block">{time}</span>
      </Link>
    ...
    return (
      <Link href={`/article/${slug}`} className="group block relative overflow-hidden rounded-lg news-card">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 md:p-8">
          {article.breakingNews && (
            <span className="live-badge mb-3 inline-flex">
              <span className="breaking-dot"></span> Breaking News
            </span>
          )}
          {catName && (
            <span className="cat-badge mb-2 inline-block">{catName}</span>
          )}
          <h2 className="font-hindi text-xl md:text-3xl font-bold leading-snug text-white group-hover:text-red-200 transition-colors line-clamp-3">
            {title}
          </h2>
          {excerpt && (
            <p className="font-hindi text-gray-200 text-sm mt-2 line-clamp-2 hidden md:block">
              {excerpt}
            </p>
          )}
  ...
  return (
    <Link href={`/article/${slug}`} className="group block news-card rounded-lg overflow-hidden bg-white border border-gray-100">
      <div className="relative aspect-video overflow-hidden bg-gray-100">
        ...
        {article.liveBlog && (
          <span className="absolute top-2 right-2 live-badge text-[10px] bg-blue-600">
            📡 Live
          </span>
        )}
      </div>
      <div className="p-3">
        {catName && (
          <Link href={`/${catSlug}`} className="cat-badge mb-2 inline-block">
            {catName}
          </Link>
        )}
        <h3 className="font-hindi font-semibold leading-snug text-gray-800 group-hover:text-primary line-clamp-3 transition-colors text-[15px]">
          {title}
        </h3>
        {excerpt && (
          <p className="font-hindi text-gray-500 text-sm mt-1 line-clamp-2">{excerpt}</p>
        )}
